  ...
  <a-list item-layout="vertical" :data-source="notifications">
    <a-list-item slot="renderItem" slot-scope="item, index" class="no-border-list">
      ...
      <a-card class="notification-card"
              :headStyle="headStyle"
              v-if="item.user_info[0]"
              :title="'来自 '+item.user_info[0].name+' 关于参与任务 【'+item.task_info[0].name+'】 的邀请'">
        ...
        <a-row>
          ...
          <a-col :span="7" :offset="1">
            ...
            <a-row v-if="item.status===0">
              <a-col :span="10">
                <a-popconfirm
                    title="你确定要接受该邀请吗？" ok-text="确认" cancel-text="取消"
                    @confirm="approve_invitation(item._id, index)">
                  <!-------------------- 接受 ------------------->
                  <a-button class="notification-button" >接受</a-button>
                </a-popconfirm>
              </a-col>
              <a-col :span="10">
                <a-popconfirm
                    title="你确定要拒绝该邀请吗？" ok-text="确认" cancel-text="取消"
